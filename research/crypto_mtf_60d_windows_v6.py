#!/usr/bin/env python3
"""60-day two-window validation for the selected LONG policies.

Fetches 60 days of Gate 5m history by backward pagination and compares the earlier
30d window with the later 30d window. This is a stronger temporal check than merely
splitting the latest 30d into two 15d halves. Research only.
"""
...
def main():
 ...
 for basket,syms in BASKETS.items():
  for s in syms:
   try:
    rows=base.fetch_gate_5m(s);print('FETCH_OK',basket,s,len(rows));coverage.append({'basket':basket,'symbol':s,'bars':len(rows)})
