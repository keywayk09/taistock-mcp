...
assert.doesNotMatch(workflow, /wrangler\s+deploy(?:\s|$)/i);
assert.doesNotMatch(workflow, /wrangler\s+versions\s+deploy(?:\s|$)/i);
assert.doesNotMatch(workflow, /wrangler\s+triggers\s+deploy(?:\s|$)/i);
