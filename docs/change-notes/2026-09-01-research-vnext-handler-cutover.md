...
- failure reason: the isolation test still globally forbids `registerTool` in every VNext file and still assumes every Production research file is unaware of VNext. Phase 10B intentionally introduces exactly one internal registration adapter in `compat-cutover.ts` and exactly one bounded `research-tools.ts` import, while Owner and all unrelated protected surfaces remain isolated.
- runtime wrapper and handler integration tests had already passed before this stale isolation assertion failed.
- downstream incremental type-check / full `test:research` / Wrangler dry-run: **SKIPPED**
- Production mutation: **NONE**

Disposition: `GREEN_ATTEMPT_2_IMMUTABLE_STALE_ISOLATION_EXPECTATION`. Authorized correction is test-only: permit registration symbols only in `src/v6/research-vnext/compat-cutover.ts`; permit VNext awareness only in `src/v6/research-tools.ts` and only through `./research-vnext/compat-cutover`; continue forbidding Owner/composition/index direct VNext knowledge and all protected-domain imports. No runtime change is authorized.

## GREEN evidence

Pending after stale isolation test-only correction.
