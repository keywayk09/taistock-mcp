...
async function tryOfficialEndpoint(
  market: Market,
  url: string,
  provider: Provider,
  source: string,
...
async function fetchMisTwseBatch(symbols: string[]) {
...
async function tryFugleTickersMisTwse(env: Env): Promise<MarketSnapshot> {
  try {
    ...
    return {
      ...
      normalized_count: rows.length,
      sample_keys: Object.keys(raw[0] ?? {}).slice(0, 24),
      errors: rows.length ? errors : [...errors, "Fugle ticker universe 可用，但 TWSE MIS listed 正規化後為 0 筆"],
    };
  } catch (error) {
    return {
