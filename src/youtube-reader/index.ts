...
const LIMITS = { timeoutMs: 12_000, watch: 3_000_000, player: 3_000_000, caption: 5_000_000, transcript: 250_000 } as const;
...
async function getText(url: string, maxBytes: number, signal: AbortSignal): Promise<string> {
	return readLimited(await fetch(url, { redirect: "error", signal, headers: { Accept: "text/html,application/json,text/plain,application/xml", "Accept-Language": "zh-TW,zh;q=0.9,en;q=0.5" } }), maxBytes);
}

async function postPlayer(url: string, body: unknown, signal: AbortSignal): Promise<any> {
	const parsed = new URL(url);
	if (parsed.origin !== "https://www.youtube.com" || parsed.pathname !== "/youtubei/v1/player") throw new YouTubeReaderError("播放器 API 端點不允許", "INVALID_HOST");
	const raw = await readLimited(await fetch(parsed, {
...
function extractJsonObject(source: string, marker: string): any {
	...
	for (let i = start; i < source.length; i++) {
		...
		if (ch === '"') inString = true; else if (ch === "{") depth++; else if (ch === "}" && --depth === 0) {
			try { return JSON.parse(source.slice(start, i + 1)); } catch { throw new YouTubeReaderError("播放器資料無法解析", "PLAYER_DATA_INVALID"); }
		}
	}
	throw new YouTubeReaderError("播放器資料不完整", "PLAYER_DATA_INVALID");
}

function configString(source: string, key: string): string | null {
...
async function innertubePlayer(html: string, videoId: string, signal: AbortSignal): Promise<any | null> {
	const key = configString(html, "INNERTUBE_API_KEY");
	if (!key || !API_KEY.test(key)) return null;
	const endpoint = new URL("https://www.youtube.com/youtubei/v1/player");
	endpoint.searchParams.set("key", key);
	endpoint.searchParams.set("prettyPrint", "false");
	return postPlayer(endpoint.toString(), {
		videoId, contentCheckOk: true, racyCheckOk: true,
		context: { client: {
			clientName: "WEB",
			clientVersion: configString(html, "INNERTUBE_CLIENT_VERSION") ?? "2.20260730.01.00",
			hl: "zh-TW", gl: "TW",
...
function chooseTrack(tracks: any[], languages: string[]) {
	const rank = (track: any) => {
		const code = String(track.languageCode ?? "").toLowerCase();
		const i = languages.findIndex((x) => code === x.toLowerCase() || code.startsWith(`${x.toLowerCase()}-`));
		return (i < 0 ? 100 : i) * 10 + (track.kind === "asr" ? 1 : 0);
	};
...
function parseCaptions(raw: string): TranscriptSegment[] {
	try {
		...
		return events.flatMap((event: any) => {
			...
			return text ? [{ startSeconds: Number(event.tStartMs ?? 0) / 1000, durationSeconds: Number(event.dDurationMs ?? 0) / 1000, text }] : [];
		});
	} catch {}
	...
	for (const match of raw.matchAll(re)) {
		const attrs = match[1];
		const startSeconds = attrs.match(/\bstart="([0-9.]+)"/)?.[1];
		const startMs = attrs.match(/\bt="(\d+)"/)?.[1];
		const durSeconds = attrs.match(/\bdur="([0-9.]+)"/)?.[1];
		const durMs = attrs.match(/\bd="(\d+)"/)?.[1];
		const text = decodeXml(match[2].replace(/<[^>]+>/g, " ")).replace(/\s+/g, " ").trim();
		if (text) segments.push({
			startSeconds: startSeconds ? Number(startSeconds) : startMs ? Number(startMs) / 1000 : 0,
			durationSeconds: durSeconds ? Number(durSeconds) : durMs ? Number(durMs) / 1000 : 0,
			text,
		});
	}
	if (segments.length === 0) throw new YouTubeReaderError("字幕資料無法解析", "CAPTION_INVALID");
	return segments;
}
...
export async function readYouTubePublicTranscript(input: string, preferredLanguages = ["zh-TW", "zh-Hant", "zh", "en"]): Promise<YouTubeReadResult> {
	...
	try {
		const html = await getText(`https://www.youtube.com/watch?v=${encodeURIComponent(videoId)}&hl=zh-TW&gl=TW&has_verified=1`, LIMITS.watch, controller.signal);
		let player: any = null;
		try { player = extractJsonObject(html, "ytInitialPlayerResponse"); } catch (error) {
		...
		if (tracks.length === 0) {
			const fallback = await innertubePlayer(html, videoId, controller.signal);
			if (fallback) { player = fallback; tracks = tracksOf(fallback); }
		}
		...
		const track = chooseTrack(tracks, preferredLanguages);
		...
		if (!(host === "youtube.com" || host.endsWith(".youtube.com") || host.endsWith(".googlevideo.com"))) throw new YouTubeReaderError("字幕來源網域不允許", "CAPTION_HOST_BLOCKED");
		captionUrl.searchParams.set("fmt", "json3");
		const segments = parseCaptions(await getText(captionUrl.toString(), LIMITS.caption, controller.signal));
		const transcript = segments.map((x) => x.text).join(" ").slice(0, LIMITS.transcript);
		return {
			videoId, url: `https://www.youtube.com/watch?v=${videoId}`,
			title: String(details.title ?? ""), author: String(details.author ?? ""),
			lengthSeconds: Number.isFinite(Number(details.lengthSeconds)) ? Number(details.lengthSeconds) : null,
			language: String(track.languageCode ?? "unknown"), captionKind: track.kind === "asr" ? "asr" : "manual",
			segments, transcript, fetchedAt: new Date().toISOString(),
		};
	} catch (error) {
