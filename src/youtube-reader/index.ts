...
const HARD_LIMITS = { timeoutMs: 12_000, maxWatchBytes: 3_000_000, maxCaptionBytes: 5_000_000, maxTranscriptChars: 250_000 } as const;
...
export async function readYouTubePublicTranscript(input: string, preferredLanguages = ["zh-TW", "zh-Hant", "zh", "en"]): Promise<YouTubeReadResult> {
	...
	try {
		const watchUrl = `https://www.youtube.com/watch?v=${encodeURIComponent(videoId)}&hl=zh-TW&has_verified=1`;
		...
		if (!Array.isArray(tracks) || tracks.length === 0) throw new YouTubeReaderError("影片沒有公開字幕或文字記錄", "NO_PUBLIC_TRANSCRIPT");
		...
		if (!captionUrl.hostname.endsWith("youtube.com") && !captionUrl.hostname.endsWith("googlevideo.com")) throw new YouTubeReaderError("字幕來源網域不允許", "CAPTION_HOST_BLOCKED");
