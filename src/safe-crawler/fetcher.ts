...
export async function safeCrawl(input: string, options: SafeCrawlerOptions = {}): Promise<SafeCrawlResult> {
	...
	while (true) {
		...
		if (declaredLength > config.maxBytes) throw new SafeCrawlerError("來源內容超過大小限制", "CONTENT_TOO_LARGE");
		...
		const extracted = contentType.includes("text/plain")
			? { title: null, text: raw.slice(0, config.maxOutputChars), markdown: raw.slice(0, config.maxOutputChars), truncated: raw.length > config.maxOutputChars }
