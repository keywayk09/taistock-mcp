...
export function registerDiamondCapabilityTools(server: McpServer) {
  server.registerTool("get_diamond_tool_registry", {
    description: "鑽石引擎正式工具列：P16 已加入 GPT Judgment / Structure / Pattern / Trendline / Trading Knowledge 長期學習記憶；OHLC Outcome 仍只透過 OHLC MCP。",
    inputSchema: {},
    annotations: { readOnlyHint:true, destructiveHint:false, idempotentHint:true, openWorldHint:false },
  }, async () => out(getDiamondToolRegistryP16()));

  server.registerTool("get_diamond_research_lab", {
    description: "鑽石引擎 Research & Validation Lab：包含台股/TXF 回測復盤、Swing 閉環，以及 P16 GPT 認知、型態與趨勢線長期統計記憶。",
    inputSchema: {},
    annotations: { readOnlyHint:true, destructiveHint:false, idempotentHint:true, openWorldHint:false },
  }, async () => out(getDiamondResearchLabP16()));
  ...
  server.registerTool("get_diamond_architecture_status", {
    description: "鑽石引擎整體狀態：資料治理、台股+TXF 復盤、Swing orchestration、Supply Chain，以及 P16 GPT Judgment→Outcome→Review→Knowledge→未來引擎規則候選的認知學習閉環。",
    inputSchema: {},
    annotations: { readOnlyHint:true, destructiveHint:false, idempotentHint:true, openWorldHint:false },
  }, async () => out(getDiamondArchitectureStatusP16()));
}
