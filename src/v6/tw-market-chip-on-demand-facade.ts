...
} from "./market-data-published-gateway.ts";
...
export const TW_MARKET_CHIP_ON_DEMAND_FACADE_VERSION = "tw-market-chip-on-demand-facade/v1.0.0";
...
/**
 * User-facing current chip facade behind the existing Owner/Family tool names.
 *
 * IMPORTANT: the formal `market-data-published-gateway` remains untouched and
 * deterministic for historical/replay research. This facade composes current
 * exact-date web evidence with the old archive only as history context. That
 * keeps production/user reads modern without contaminating formal replay tests.
 */
export async function getTwMarketChipSummaryOnDemand(env: Env, input: PublishedGatewayInput) {
  ...
  return {
    ...
    version: TW_MARKET_CHIP_ON_DEMAND_FACADE_VERSION,
    provider_versions: {
    ...
    legacy_consistency: legacyRecord.consistency ?? "PUBLISHED",
    ...
    migration_note: "Current chip data is fetched on demand. Formal published gateway stays deterministic for replay. Warrant activity is official but non-directional; ranked broker evidence is secondary/fail-soft.",
  };
}
