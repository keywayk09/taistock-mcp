...
export function registerSharedCryptoMarketTools(server: McpServer, env: Env) {
  server.registerTool("get_crypto_engine_status", {
    description: "讀取中央幣圈引擎健康狀態、穩健/妖幣模式與多週期能力。正式選幣固定搭配日線、4小時、1小時、15分、5分：日線看主方向、4H看波段結構、1H看setup背景、15m看結構、5m只負責觸發。唯讀、不下單、不改參數。",
    inputSchema: {},
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, async () => out(await fetchCryptoJson(env, "/health")));

  server.registerTool("get_crypto_candidates", {
    description: "從中央幣圈引擎即時取得做多/做空候選。所有候選都必須搭配1D/4H/1H/15m/5m，禁止只用5m/15m小週期決策。profile=stable_quality 是較穩健、流動性與跨交易所品質優先；profile=volatile 是妖幣/高波動模式，細分順勢追、回踩量縮、整理待突破、擠壓加速與高波觀察。使用者說『穩定幣/穩一點』時用 stable_quality；說『妖幣/小幣/波動大』時用 volatile。這裡的穩定幣是交易風格稱呼，不是USDT/USDC stablecoin。唯讀，不代表自動進場訊號。",
    inputSchema: {
      profile: z.enum(["stable_quality", "volatile"]).optional().default("stable_quality"),
      setup: z.enum(["all", "trend_follow", "pullback_contraction", "consolidation", "squeeze", "unclassified"]).optional().default("all"),
  ...
  server.registerTool("get_crypto_deep_probe", {
    description: "對指定1到6個幣做中央幣圈引擎完整多週期Deep Probe。固定讀取日線、4H、1H、15m、5m；同時包含Bybit/Gate價格、成交量、OI與Price×OI regime。5m只當觸發層，不可單獨形成結論。唯讀，不建立部位。",
    inputSchema: {
