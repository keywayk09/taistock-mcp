...
export async function getTwMarketChipSummaryPublished(env: Env, input: PublishedGatewayInput) {
  ...
  for (const month of months) {
    ...
    if (month === publishedMonth) {
      const path = marketReadPublishedShardPath(pointer.trade_date, pointer.generation, prefix);
      const read = await readGitHubJson<MarketReadShardReceipt>(env, path);
      if (!read.value) return unavailable(input, `published_shard_missing:${prefix}`, pointer);
      try {
        assertPublishedShard(pointer, read.value);
      } catch (error) {
        return unavailable(input, `published_shard_invalid:${String(error)}`, pointer);
      }
      state = (read.value.symbols?.[input.symbol] ?? {}) as Partial<Record<TwMarketDataKind, any[]>>;
      datasets.push({ path: read.path, sha: read.sha, role: "PUBLISHED_GENERATION_SHARD" });
    } else {
