...
export const RESEARCH_VNEXT_REPLAY_IMPLEMENTATION_VERSION = "research-vnext-selective-1m-replay/v1.0.0" as const;

// Public replay identity remains frozen during architecture migration so GPT and
// existing tools receive byte-for-byte equivalent deterministic evidence.
const SELECTIVE_1M_REPLAY_ENGINE_VERSION = "diamond-selective-1m-replay/v1.0.0";
const SELECTIVE_1M_REPLAY_SCHEMA_VERSION = "diamond-1m-replay-result/v1";
...
export type ResearchVNextIntraday1mBar = Intraday5mBar & {
  trade_date?: string;
  ingest_id?: string;
  export_batch?: string;
...
export type ResearchVNextSelectiveReplayInput = {
  original_5m_result: DeterministicBacktestResult;
  dataset_1m: FrozenDatasetManifest;
  bars_1m: ResearchVNextIntraday1mBar[];
};

export class ResearchVNextSelectiveReplayError extends Error {
  readonly code: string;
  readonly detail?: Record<string, unknown>;

  constructor(code: string, message: string, detail?: Record<string, unknown>) {
    super(message);
    this.name = "ResearchVNextSelectiveReplayError";
    this.code = code;
    this.detail = detail;
  }
}
...
function finite(value: unknown, field: string): number {
  ...
  if (!Number.isFinite(number)) {
    throw new ResearchVNextSelectiveReplayError("INVALID_INPUT", `${field} must be finite`);
  }
  return number;
}
...
function positive(value: unknown, field: string): number {
  ...
  if (number <= 0) {
    throw new ResearchVNextSelectiveReplayError("INVALID_INPUT", `${field} must be > 0`);
  }
  return number;
}
...
function canonicalRows(rows: ResearchVNextIntraday1mBar[]) {
  return rows.map((row) =>
    OHLC_1M_COLUMNS.map((key) => {
      const value = row[key];
...
async function recomputeDatasetHash(
  dataset: FrozenDatasetManifest,
  rows: ResearchVNextIntraday1mBar[],
): Promise<string> {
...
function tradeDateOf(row: ResearchVNextIntraday1mBar): string {
...
function validateDataset(
  dataset: FrozenDatasetManifest,
  rows: ResearchVNextIntraday1mBar[],
  original: DeterministicBacktestResult,
) {
  if (!dataset || dataset.schema_version !== "ohlc-dataset/v1") {
    throw new ResearchVNextSelectiveReplayError("INVALID_DATASET", "1m dataset must use ohlc-dataset/v1");
  }
  if (!dataset.frozen_view || !dataset.complete_view || dataset.truncated || !dataset.formal_research_eligible) {
    throw new ResearchVNextSelectiveReplayError(
      "DATASET_NOT_ELIGIBLE",
      "1m replay requires complete formal frozen dataset",
    );
  }
  if (dataset.provenance?.market !== "tw-stock" || dataset.provenance?.timeframe !== "1m") {
    throw new ResearchVNextSelectiveReplayError(
      "INVALID_DATASET",
      "selective replay requires tw-stock 1m dataset",
    );
  }
  if (String(dataset.provenance?.symbol ?? "") !== original.symbol) {
    throw new ResearchVNextSelectiveReplayError(
      "SYMBOL_MISMATCH",
      "1m dataset symbol differs from original 5m result",
    );
  }
  if (Number(dataset.row_count) !== rows.length || Number(dataset.total_validated_rows) !== rows.length) {
    throw new ResearchVNextSelectiveReplayError(
      "DATASET_ROW_COUNT_MISMATCH",
      "bars_1m must be the exact complete frozen dataset view",
    );
  }
  if (
    `sha256:${dataset.dataset_hash}` !== dataset.dataset_version ||
    !/^[0-9a-f]{64}$/.test(String(dataset.dataset_hash ?? ""))
  ) {
    throw new ResearchVNextSelectiveReplayError(
      "INVALID_DATASET_VERSION",
      "1m dataset version/hash mismatch",
    );
  }
  if (!rows.length) {
    throw new ResearchVNextSelectiveReplayError("DATASET_EMPTY", "1m bars are empty");
  }
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (String(row.symbol) !== original.symbol) {
      throw new ResearchVNextSelectiveReplayError(
        "SYMBOL_MISMATCH",
        `bars_1m[${index}] symbol mismatch`,
      );
    }
    const ts = finite(row.ts_ms, `bars_1m[${index}].ts_ms`);
    if (seen.has(ts)) {
      throw new ResearchVNextSelectiveReplayError("DUPLICATE_BAR", `duplicate 1m ts ${ts}`);
    }
    if (ts <= previous) {
      throw new ResearchVNextSelectiveReplayError("UNSORTED_BARS", "1m bars must be strictly chronological");
    }
    seen.add(ts);
    previous = ts;

    const open = positive(row.open, `bars_1m[${index}].open`);
    const high = positive(row.high, `bars_1m[${index}].high`);
    const low = positive(row.low, `bars_1m[${index}].low`);
    const close = positive(row.close, `bars_1m[${index}].close`);
    const volume = finite(row.volume, `bars_1m[${index}].volume`);
    if (
      volume < 0 ||
...
/**
 * Resolve only an already-ambiguous deterministic 5m result using an exact,
 * complete frozen 1m dataset. This is evidence computation, not trading
 * reasoning: GPT remains the sole interpretation and decision owner.
 */
export async function resolveAmbiguousBacktestEvidenceWith1m(
  input: ResearchVNextSelectiveReplayInput,
) {
  const original = input?.original_5m_result;
  if (!original || original.status !== "OK" || !original.deterministic) {
    throw new ResearchVNextSelectiveReplayError(
      "INVALID_5M_RESULT",
      "original deterministic 5m result is required",
    );
  }
  if (
    !original.ambiguous_intrabar ||
    !original.requires_1m_replay ||
    original.intrabar_status !== "AMBIGUOUS_INTRABAR"
  ) {
    throw new ResearchVNextSelectiveReplayError(
      "REPLAY_NOT_REQUIRED",
      "P6 only accepts 5m results explicitly marked for selective 1m replay",
    );
  }
  if (original.conservative_resolution !== "STOP_FIRST" || original.exit_reason !== "STOP") {
    throw new ResearchVNextSelectiveReplayError(
      "INVALID_5M_RESULT",
      "ambiguous 5m baseline must retain conservative STOP_FIRST result",
    );
  }
  ...
  if (recomputed !== input.dataset_1m.dataset_hash) {
    throw new ResearchVNextSelectiveReplayError(
      ...
      "1m bars/source provenance do not reproduce dataset_hash",
      { expected: input.dataset_1m.dataset_hash, recomputed },
    );
  }
  ...
  if (!bucket.length) {
    throw new ResearchVNextSelectiveReplayError(
      "REPLAY_BUCKET_NOT_FOUND",
      "1m dataset contains no bars in ambiguous 5m bucket",
      { bucket_start_ts_ms: bucketStart, bucket_end_exclusive_ts_ms: bucketEnd },
    );
  }
  ...
  let resolutionBar: ResearchVNextIntraday1mBar | null = null;
  ...
  if (!resolution || !resolutionBar) {
    throw new ResearchVNextSelectiveReplayError(
      "REPLAY_INCONSISTENT_WITH_5M",
      "1m bars did not reproduce either stop or target touch inside a 5m bar that was marked ambiguous",
      { backtest_run_id: original.backtest_run_id, bucket_start_ts_ms: bucketStart },
    );
  }

  const resolvedExitReason = resolution === "TARGET" ? "TARGET" : "STOP";
  const resolvedExitPrice = resolution === "TARGET" ? target : stop;
  const direction = original.side === "LONG" ? 1 : -1;
  const gross = ((resolvedExitPrice - original.entry_price) / original.entry_price) * direction * 100;
