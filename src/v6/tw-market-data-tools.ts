...
export function registerTwMarketDataTools(server: McpServer, env: Env) {
  server.registerTool("get_tw_market_data_contract", {
    ...
    storage: "Cloudflare RESEARCH_DB index + RESEARCH_BUCKET immutable snapshot",
  ...
  server.registerTool("get_tw_institutional_flow", {
    description: "查詢台股個股三大法人 1/3/5/10/20 日累積。Diamond 官方已封存資料優先，FinMind僅補歷史/降級。",
    inputSchema: {
      symbol: symbolSchema,
  ...
  server.registerTool("get_tw_margin_short", {
    description: "查詢台股個股融資融券餘額與 1/3/5/10/20 日變化。Diamond 官方已封存資料優先，FinMind僅補歷史/降級。",
    inputSchema: {
      symbol: symbolSchema,
      as_of: dateSchema.optional(),
      calendar_days: z.number().int().min(30).max(180).optional().default(60),
  ...
  server.registerTool("get_tw_market_data_status", {
    description: "查詢 Diamond 台股官方法人/融資融券四層封存 readiness；此狀態永遠不作為 OHLC 全域 blocker。",
